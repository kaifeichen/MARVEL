...
std::vector<std::pair<int, Transform>> Apriltag::aprilLocalize(
  ...
  for (unsigned int i = 0; i < tagPoseInCamFrame.size(); i++) {
    ...
    if (tagPoseInModelFrame[i].first >= 0) {
      camPoseInModelFrame.push_back(std::make_pair(
          tagPoseInModelFrame[i].first,
          tagPoseInModelFrame[i].second * tagPoseInCamFrame[i].inverse()));
    }
  }

  long totalTime = Utility::getTime() - totalStartTime;
  std::cout << "Time Apriltag localize overall " << totalTime << " ms"
            << std::endl;
...
Apriltag::aprilDetect(const cv::Mat &im, const CameraModel &camera) {
  std::vector<Transform> tagPoseInCamFrame;
  ...
  long totalTime = Utility::getTime() - totalStartTime;
  std::cout << "Time Apriltag detect overall " << totalTime << " ms"
            << std::endl;

  return std::pair<std::vector<int>, std::vector<Transform>>(tagCodes,
                                                             tagPoseInCamFrame);
}

Transform
Apriltag::calculateNewAprilTagPoseInModelFrame(Transform camPoseInModelFrame,
